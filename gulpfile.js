...
function checkConfig(cb) {
	if (!SECRET_KEY) {
		cb(
			new Error(
				`Не задан ${chalk.red(`SECRET_KEY`)} в файле ` + chalk.red(`storeland-uploader-config.json`)
			)
		)
	}
	if (!SITE) {
		cb(
			new Error(
				`Не задан url адрес cайта в параметре ${chalk.red(`SITE`)} в файле ` +
...
function scripts() {
	return src(paths.scripts.src).pipe(wait(WAIT_TIME)).pipe(browserSync.stream())
}
...
function styles() {
	if (preprocessorOn) {
		...
		return src(paths.styles.src).pipe(wait(WAIT_TIME)).pipe(browserSync.stream())
	}
}
		...
		.then(encoded => {
			...
			fetch(href, {
				...
				.then(json => {
					if (json.status === `ok`) {
						console.log(
							`[${moment().format('HH:mm:ss')}] Файл ${chalk.red(fileName)} успешно отправлен ✔️`
						)
